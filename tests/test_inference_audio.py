...
from rift_svc.inference.audio import add_segment, write_audio


def test_add_segment_preserves_float32_output() -> None:
    segments = [
...
def test_add_segment_applies_boundary_fades() -> None:
    ...
    add_segment(
        ...
        total_segments=2,
        start_sample=0,
        audio_out=np.ones(6, dtype=np.float32),
        expected_length=6,
        fade_samples=2,
    )
    add_segment(
        ...
        total_segments=2,
        start_sample=6,
        audio_out=np.full(4, 2.0, dtype=np.float32),
        expected_length=4,
        fade_samples=2,
    )

    np.testing.assert_allclose(result, [1, 1, 1, 1, 1, 0, 0, 1.5, 2, 2])
